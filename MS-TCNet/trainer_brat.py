...
def train_epoch(model,
    ...
    for idx, batch_data in enumerate(loader):
        ...
        with autocast(enabled=args.amp):
            fi_out,logits,logits1,logits2,logits3,logits4,logits5 = model(data)

            loss0 = loss_func(fi_out, target)
            loss1 = loss_func(logits, target)
            loss2 = loss_func(logits1, target)
            loss3 = loss_func(logits2, target)
            loss4 = loss_func(logits3, target)
            loss5 = loss_func(logits4, target)
            loss6 = loss_func(logits5, target)
...
def val_epoch(model,
    ...
    with torch.no_grad():
        for idx, batch_data in enumerate(loader):
            ...
            acc = acc_func(y_pred=val_output_convert, y=val_labels_list)
            ...
            if args.rank == 0:
                ...
                for m,accValue in enumerate(acc_list):
                    print(accValue," ")
            averageAcc.append(avg_acc)
            start_time = time.time()
    return np.mean(averageAcc)
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if (epoch+1) % args.val_every == 0:
            ...
            if args.rank == 0 and args.logdir is not None and args.save_checkpoint:
                ...
                if b_new_best:
                    print('Copying to model.pth new best model!!!!')
