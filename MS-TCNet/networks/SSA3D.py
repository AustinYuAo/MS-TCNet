    ...
    def forward(self, x, H, W,Z):
        ...
        if self.sr_ratio > 1:
                ...
                v1 = v1 + self.local_conv1(v1.transpose(1, 2).reshape(B, -1, C//2).
                                        transpose(1, 2).view(B,C//2, H//self.sr_ratio, W//self.sr_ratio, Z//self.sr_ratio)).\
                ...
                v2 = v2 + self.local_conv2(v2.transpose(1, 2).reshape(B, -1, C//2).
                                        transpose(1, 2).view(B, C//2, H*2//self.sr_ratio, W*2//self.sr_ratio,Z*2//self.sr_ratio)).\
                    view(B, C//2, -1).view(B, self.num_heads//2, C // self.num_heads, -1).transpose(-1, -2)
...
class OverlapPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=96, patch_size=7, stride=4,in_chans=1, embed_dim=768,dropout_rate: float = 0.0):
        ...
        self.H, self.W, self.Z = img_size[0] // patch_size[0], img_size[1] // patch_size[1],img_size[2] // patch_size[2]

        self.num_patches = self.H * self.W * self.Z
...
class Head(nn.Module):
    def __init__(self,channels=1,num=2,dropout_rate: float = 0.0):
        ...
        stem = [nn.Conv3d(channels, 32, 7, 2, 3, bias=False),nn.BatchNorm3d(32), nn.ReLU(True)]
        ...
        stem1.append(nn.Conv3d(64, 64, kernel_size=2, stride=2))
...
class ShuntedTransformer(nn.Module):
    def __init__(self, img_size=96, in_chans=3,  classification: bool = False,num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        for i in range(num_stages):
            if i ==0:
                patch_embed = Head(channels=in_chans,num=num_conv,dropout_rate=0.0)
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages):
            ...
            if i ==0:
                ...
                x0 = x0.reshape(B, H*2, W*2, Z*2, -1).permute(0, 4, 1, 2, 3).contiguous()
